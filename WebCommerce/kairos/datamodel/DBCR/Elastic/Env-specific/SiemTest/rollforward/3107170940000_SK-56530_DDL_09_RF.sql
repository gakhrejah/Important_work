...
BEGIN
	...
	insert into sk_category_data(category_id, last_modified) select cat.category_id, last_mod_date from dcs_category@DBLNK_45_TO_44 cat left join sk_category@DBLNK_45_TO_44 skcat on cat.category_id=skcat.category_id;
    ...
    insert into sk_category_paths(category_id, category_path) select CHILD_CAT_ID, SYS_CONNECT_BY_PATH(category_id, '/') "Path" from DCS_CAT_CHLDCAT@DBLNK_45_TO_44 CONNECT BY  category_id = PRIOR CHILD_CAT_ID start WITH CATEGORY_ID not in (select CHILD_CAT_ID from DCS_CAT_CHLDCAT@DBLNK_45_TO_44);
	...
	insert into sk_category_name(category_id, language, display_name) select mapTable.CATEGORY_ID, mapTable.LANGUAGE,langInfo.DISPLAY_NAME from SK_CAT_LANGUAGE_INFO@DBLNK_45_TO_44 langInfo , sk_Cat_language@DBLNK_45_TO_44 mapTable where langInfo.id =mapTable.CAT_LANGUAGE_INFO;
	...
	insert into sk_product_data(product_id, last_modified, ean, upc) select UPPER(prd.product_id), last_mod_date, ean, upc from dcs_product@DBLNK_45_TO_44 prd left join sk_product@DBLNK_45_TO_44 skprd on prd.product_id=skprd.product_id;
	...
	insert into sk_product_name(product_id, language, display_name) select UPPER(prdlang.product_id), prdlang.language, langinfo.display_name from sk_prd_language_info@DBLNK_45_TO_44 langInfo, sk_prd_language@DBLNK_45_TO_44 prdLang where prdlang.prd_language_info = langinfo.id;
